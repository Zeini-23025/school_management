...
const Classes: React.FC = () => {
  ...
  const handleDelete = (id: string) => {
    ...
    if (count > 0) {
      if (!window.confirm(t('delete_class_confirm_students', count.toString()))) return;
    } else {
  ...
  const getLevelName = (lvl: number) => {
    if (lvl >= 1 && lvl <= 6) {
       return t(`level_${lvl}` as any);
    }
    return t('level_x', lvl.toString());
  };
              ...
              >
                <div className="flex items-center gap-3">
                   ...
                   <div>
                      ...
                      <p className="text-sm text-gray-500">{levelClasses.length} {t('groups')}</p>
                   </div>
                </div>
                <div className="flex items-center gap-3">
                  {!isTeacher && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all scale-100">
             <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
               <h3 className="text-xl font-bold text-gray-800">{editingId ? t('edit_class') : t('new_class')}</h3>
             ...
             <div className="p-6 space-y-4">
                <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 mb-2">
                   <p className="text-sm text-blue-800 font-medium">
                     {t('adding_in_level', getLevelName(formData.level || 1))}
                   </p>
                </div>
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-sm font-medium text-gray-700">{t('change_level')}</label>
                  <select 
             ...
             <div className="p-6 border-t border-gray-100 flex justify-end gap-3 bg-gray-50">
               <button onClick={() => setShowModal(false)} className="px-5 py-2.5 text-gray-600 hover:bg-gray-200 rounded-xl font-medium transition">{t('cancel')}</button>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
             <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
               <div>
                 ...
                 <p className="text-sm text-gray-500 mt-1">{t('level_x', selectedClassForView.level.toString())} | {t('student_count')}: {getClassStudents().length}</p>
               </div>
                  ...
                  <table className={`w-full ${language === 'ar' ? 'text-right' : 'text-left'}`}>
                    <thead className="bg-gray-50 sticky top-0">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-xs font-semibold text-gray-500 uppercase">{t('full_name')}</th>
                        <th className="px-6 py-3 text-xs font-semibold text-gray-500 uppercase">{t('birth_date')}</th>
                        <th className="px-6 py-3 text-xs font-semibold text-gray-500 uppercase">{t('gender')}</th>
                        <th className="px-6 py-3 text-xs font-semibold text-gray-500 uppercase">{t('parent_phone')}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {getClassStudents().map((s, idx) => (
                        <tr key={s.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 text-gray-600 text-sm">{s.gender === 'M' ? t('male') : t('female')}</td>
